...
const App: React.FC = () => {
  ...
  const generateAIImage = async () => {
    if (!formExercise.name) {
      alert('يرجى إدخال اسم التمرين أولاً لتوليد صورة مناسبة');
      return;
    }
    ...
    try {
      ...
      const prompt = `A clean, minimalist 3D illustration or high-quality gym diagram showing the workout exercise: "${formExercise.name}". Target muscle: ${formExercise.muscleType}. White background, professional fitness app style.`;
      ...
      for (const part of response.candidates[0].content.parts) {
        if (part.inlineData) {
          ...
          setFormExercise(prev => ({ ...prev, image: imageUrl }));
          break;
        }
      }
    } catch (error) {
          ...
          <div className="space-y-4">
            {activeDay.exercises.map(ex => (
              <ExerciseCard 
                ...
                onImageClick={() => setPreviewImage(ex.image || null)}
              />
            ))}
          </div>
        )}
        ...
        <div className="flex items-center gap-4 bg-white dark:bg-slate-800 p-4 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700">
          {activeExercise.image ? (
            <img 
              ...
              className="w-16 h-16 rounded-2xl object-cover cursor-pointer hover:opacity-80 transition-opacity" 
              onClick={() => setPreviewImage(activeExercise.image || null)}
            />
          ) : (
              ...
              <label htmlFor="img-upload" className={`flex-1 p-6 border-2 border-dashed rounded-[1.5rem] text-center cursor-pointer transition-all ${darkMode ? 'border-slate-700 bg-slate-900/50' : 'border-gray-200 bg-gray-50/50'}`}>
                {formExercise.image ? (
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    <span className="text-xs font-bold text-gray-400">تحميل من الاستوديو</span>
                  </div>
                )}
              </label>
              {formExercise.image && (
                <button onClick={() => setFormExercise(p => ({ ...p, image: undefined }))} className="w-14 h-14 bg-red-50 text-red-500 border border-red-100 rounded-2xl flex items-center justify-center active:scale-95 transition-transform">
      ...
      {previewImage && (
        ...
        >
          <div className="relative max-w-full max-h-full">
            ...
            <img 
              src={previewImage} 
              className="rounded-3xl shadow-2xl max-w-full max-h-[80vh] object-contain border-4 border-white/10" 
              alt="Preview" 
            />
          </div>
        </div>
